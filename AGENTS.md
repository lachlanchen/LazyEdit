...
- After pushing AutoPublish changes from this repo, pull them on the Raspberry Pi:
  ...
  ```
- Runtime secrets for the live AutoPublish host now live in:
  - `/home/lachlan/Projects/autopub/.env`
- AutoPublish runtime should treat repo `.env` as the default secret source, with `.bashrc` only as fallback.
