#!/usr/bin/env python3
"""
French language enhancement plugin (independent of legacy enhancements).

Behavior:
- One-pass: translate/polish to French, then analyze into coarse grammar tokens.
- No English gloss; render colorized POS tokens consistent with other languages.

Output:
- EnhancementResult.processed_text: French text (normalized/translated/polished)
- EnhancementResult.display_html: inline colored tokens
- EnhancementResult.tts_language_code: 'fr'
...
class FrenchEnhancer:
    ...
    def __init__(self, max_retries: int = 2):
        ...
        self.client, self.provider_mode = build_with_fallback(
            use_cache=True,
            cache_dir='cachexxx/french_enhancement_cache',
            max_retries=max_retries
        )
        self.schema = {
            "type": "object",
            "properties": {
                "french_text": {"type": "string"},
                "words": {
                    "type": "array",
                    "items": {
                        "type": "object",
            ...
            "required": ["french_text","words"],
            "additionalProperties": False
        }

    def _detect_language(self, text: str) -> str:
        try:
            if any(ch in text for ch in "àâäçéèêëîïôöùûüÿÀÂÄÇÉÈÊËÎÏÔÖÙÛÜŸ"):  # rough heuristic
                return 'fr'
        except Exception:
            pass
        return 'unknown'
    ...
    def _build_prompt(self, text: str) -> str:
        return (
            f"If the text: \"{text}\" is not French, translate it into natural French (with correct diacritics) first.\n"
            f"If it's already French, polish it to correct and native French.\n\n"
            "Analyze this French text into words.\n\n"
            "Return JSON with:\n"
            "- french_text: native and exact French translation or polished original French\n"
            "- words: array of objects with:\n"
            "  - word: the word/particle/punctuation as it appears in french_text\n"
    ...
    def _render_html(self, data: Dict[str, Any]) -> str:
        ...
        for t in tokens:
            ...
            attrs = [f'class="fr-token pos-{pos}"', f'style="--pos-color:{color};"']
            inline.append(f'<span {' '.join(attrs)}>{w}</span>')
        if inline:
            parts.append('<div class="fr-inline">' + ' '.join(inline) + '</div>')
        return '<div class="french-enhancement">' + ''.join(parts) + '</div>'
        ...
        try:
            ...
            try:
                resp = self.client.send_request_with_json_schema(
                    ...
                    system_content="You are a precise French linguistic analyzer. Output strict JSON only.",
                    schema_name="french_enhancement_v1",
                )
            except Exception:
            ...
            data = {
                'french_text': resp.get('french_text') or text,
                'tokens': toks,
            }
            html_out = self._render_html(data)
            return EnhancementResult(
                original_text=text,
                processed_text=data.get('french_text') or '',
            ...
            try:
                logging.getLogger(__name__).error(f"[FR] enhancement error: {e}")
            except Exception:
                pass
            return EnhancementResult(
                original_text=text,
