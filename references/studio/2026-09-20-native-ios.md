...
Build **1.0 (3)**, `art.lazying.lazyedit`, passed Apple validation and was
uploaded exactly once. Apple build/delivery
`36e6e0d8-4793-42a3-a7f4-0e8a870ff3ba` is **VALID / IN_BETA_TESTING**, attached
to the existing private internal group with one owner tester. No public beta
or App Store production submission was made. The SHA256 and source commit
are in `store/studio/release.json`. Install the update from the owner's existing
TestFlight invitation/app; native login is separate from the old WebView session.

The IPA, provider receipts, screenshots and runtime/cleanup handoff are saved
privately under Nutstore `Share/LazyEdit/runtime/2026-09-20-native-ios`. The
existing Studio account file remains in `Share/LazyEdit/studio-account.json`.
The dedicated review desktop and simulator were stopped after QA; the shared
store browser and another project's simulator were preserved.
