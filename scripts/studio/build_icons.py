#!/usr/bin/env python
"""Build Studio application icons; never modify the video watermark asset."""
...
with tempfile.TemporaryDirectory() as d:
 ...
 for dest,size in [(r/'studio/web/icon.png',1024)]+[(r/f'mobile/android/app/src/main/res/mipmap-{density}/{name}.png',size) for density,size in [('mdpi',48),('hdpi',72),('xhdpi',96),('xxhdpi',144),('xxxhdpi',192)] for name in ['ic_launcher','ic_launcher_round']]:
...
# Native iOS design is separate from the established video watermark.
for name, size in [("AppIcon.appiconset/StudioRibbon-v1.png", 1024),
                   ("StudioMark.imageset/StudioMark.png", 256)]:
 dest = r / "mobile/ios/App/App/Assets.xcassets" / name
 dest.parent.mkdir(parents=True, exist_ok=True)
 subprocess.run(["convert", str(r / "figs/app-icon/lazyedit-studio-ribbon-v1.png"),
                 "-resize", f"{size}x{size}", "-alpha", "off", "PNG24:" + str(dest)], check=True)
