...
c=json.loads((p/'worker.json').read_text());c.update(webRoot=str(release/'studio/web'),staticRoot=str(release/'webdist'),python=sys.executable,sourceRoot=str(r));(p/'worker.json').write_text(json.dumps(c,indent=2))
